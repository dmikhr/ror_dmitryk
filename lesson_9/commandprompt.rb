...
# здесь в методах добавлен пользовательский ввод, этот класс
# используется в меню программы, таким образом разделяется пользовательский
# ввод и логика работы соответствующих методов
# в классе CommandPrompt реализуется ввод, а обработка
# происходит через super() в методе родительского класса
# также родительский класс Command, за счет отсутствия пользовательского ввода,
# можно использовать для автоматического тестирования методов (test.rb)
class CommandPrompt < Command
  attr_reader :commands

  def initialize
    # команды задаюстся в виде 2-х мерного массива,
    # парами название_команды - метод
    @commands = [
      ['Создать станцию', method(:create_station)],
      ['Создать поезд', method(:create_train)],
      ['Создать маршрут', method(:create_route)],
      ['Добавить станцию в маршрут', method(:add_station_to_route)],
      ['Удалить станцию из маршрута', method(:remove_station_from_route)],
      ['Назначить маршрут поезду', method(:set_route)],
      ['Прицепить вагон к поезду', method(:add_carriage)],
      ['Отцепить вагон от поезда', method(:remove_carriage)],
      ['Переместить поезд по маршруту', method(:move_train)],
      ['Список станций', method(:show_available_stations)],
      ['Cписок поездов на станции', method(:trains_on_station)],
      ['Список вагонов у поезда', method(:train_carriages_list)],
      ['Зарезервировать место в пассажирском вагоне', method(:reserve_seat)],
      ['Зарезервировать объем в грузовом вагоне', method(:reserve_volume)],
  ...
  # вывод в цикле может быть удобнее по сравнению с case/when
  # в случае необходимости добавления новых команд или изменения
  # порядка их вывода
  def show_options
  ...
  def train_carriages_list(train_number = '', _type = '')
  ...
  def reserve_seat(carriage_id = '')
    puts 'Введите номер пассажирского вагона в котором "\
    "нужно зарезервировать место'
