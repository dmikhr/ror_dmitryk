...
class Route
  ...
  def validate!
    unless @stations.first.class == Station && @stations.last.class == Station
      raise 'Для создания маршрута необходимы объекты класса Station'
    end
    if @stations.first == @stations.last
      raise 'Для создания маршрута необходимы 2 разные станции'
    end
  end
end
